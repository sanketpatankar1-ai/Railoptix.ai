...
export default function Schedules() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Clear filters so the new schedules are visible!
      setSearchTerm('');
      setFilterSection('');
      setFilterDept('');
      ...
      showToast(`RailOptix: Generated ${count} ${planType.toUpperCase()} block schedules.`);
    } catch (err) {
  ...
  const handleApprove = async (id) => {
    try {
      ...
      showToast('Approval failed: ' + (err.response?.data?.detail || err.response?.data?.message || err.message), 'error');
    }
  };
  ...
  const handleExport = async (format) => {
    try {
      if (format === 'xlsx') {
        ...
        a.href = url;
        a.download = `railoptix_schedules_${planType}.xlsx`;
        a.click();
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1F2937', margin: '0 0 4px' }}>
            RailOptix Automatic Block Planning & Schedule Optimizer
          </h2>
