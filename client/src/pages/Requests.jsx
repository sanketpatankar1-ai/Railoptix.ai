  ...
  const handleSubmitRequest = async (e) => {
    ...
    try {
      ...
      await taskAPI.create({
        task_id: `REQ-${Date.now()}`,
        source_system: form.department === 'Engineering' ? 'TMS' : form.department === 'Signal & Telecom' ? 'SMMS' : 'TDMS',
        department: form.department,
        section_id: form.sectionId,
        section_name: section?.sectionName || section?.section_name || form.sectionId,
        defect_type: form.defectType,
        defect_description: form.description,
        criticality: form.criticality,
        reported_date: new Date().toISOString().split('T')[0],
        due_date: new Date(Date.now() + 7 * 86400000).toISOString().split('T')[0],
        estimated_duration: parseInt(form.estimatedDuration),
        status: 'pending',
      });
      setShowForm(false);
      setForm({ sectionId: '', department: user?.department || '', defectType: '', estimatedDuration: 60, criticality: 'medium', description: '' });
      showToast('Block request submitted successfully to RailOptix');
    } catch (err) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: '700', color: '#1F2937', margin: '0 0 4px' }}>
            RailOptix Block Requests & Approval
          </h2>
