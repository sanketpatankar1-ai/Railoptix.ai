...
def _build_system_prompt(db: Session, user_query: str = "", history: list[ChatMessage] = None, zone: str = None) -> tuple[str, list[str], list[str]]:
    ...
    system_prompt = f"""You are RailOptix Assistant — the official Indian Railways Intelligent Maintenance Planning Advisor.
...
def chat(
    ...
    return ChatResponse(
        success=True,
        response=(
            f"### RailOptix Operational Briefing{zone_label}\n\n"
